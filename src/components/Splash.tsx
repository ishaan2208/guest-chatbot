...
interface SplashProps {
  /** Optional supporting line (replaces the default tagline for context). */
  sub?: string;
...
/** Deterministic pseudo-random so the ember field is stable across renders. */
...
/**
 * The Zenvana brand splash — a faithful port of the sister app's InitialSplash:
 * gold embers rising from a hearth glow, an iris bloom behind the logo, the
 * lotus mark de-blurring in and breathing, on near-black. Presentational: the
 * parent controls when it's mounted (post-login, booking fetch, QR sign-in).
 */
export default function Splash({ sub }: SplashProps) {
  ...
  return (
    <div
      ...
      role="status"
      className="zv-splash fixed inset-0 z-[260] flex items-center justify-center overflow-hidden"
    >
      ...
      {/* Soft vignette so the logo region feels luminous */}
      ...
      <div className="relative z-10 px-6 text-center text-white">
        ...
        <div className="zv-logo-wrap mx-auto mt-4">
          <img
            ...
            width={320}
            height={320}
            className="zv-logo mx-auto h-auto w-[clamp(150px,34vw,240px)]"
            decoding="async"
          />
        </div>

        <p className="zv-tagline mt-3 text-xs text-white/70 sm:text-sm">
          {sub ?? "Boutique stays on Rajpur Road"}
        </p>
...
@keyframes zv-hearth-pulse { 0% { opacity: 0.65; } 100% { opacity: 1; } }
...
@keyframes zv-title-in { to { opacity: 1; transform: translateY(0); } }

.zv-logo-wrap { position: relative; display: inline-block; isolation: isolate; }
.zv-logo-wrap::before {
  content: '';
  position: absolute;
  inset: -24%;
  ...
  background: radial-gradient(closest-side,
    rgba(232,214,147,0.24), rgba(232,214,147,0.05) 55%, transparent 70%);
  filter: blur(20px);
  opacity: 0;
  z-index: -1;
  animation:
...
@keyframes zv-halo-in   { 0% { opacity: 0; transform: scale(0.7); } 100% { opacity: 0.9; transform: scale(1); } }
@keyframes zv-halo-breathe { 0% { opacity: 0.8; transform: scale(1); } 100% { opacity: 1; transform: scale(1.07); } }
...
@keyframes zv-logo-in {
  ...
  100% { opacity: 1; transform: translateY(0) scale(1);
         filter: drop-shadow(0 0 22px rgba(232,214,147,0.38)) drop-shadow(0 0 60px rgba(232,214,147,0.22)) blur(0); }
}
@keyframes zv-logo-breathe {
  0%   { transform: translateY(0) scale(1);
         filter: drop-shadow(0 0 22px rgba(232,214,147,0.32)) drop-shadow(0 0 60px rgba(232,214,147,0.18)) blur(0); }
  100% { transform: translateY(-3px) scale(1.012);
         filter: drop-shadow(0 0 30px rgba(232,214,147,0.46)) drop-shadow(0 0 82px rgba(232,214,147,0.28)) blur(0); }
}
...
@keyframes zv-tagline-in { to { opacity: 1; transform: translateY(0); } }
...
@keyframes zv-rule-draw { to { width: 8rem; } }

.zv-dots { opacity: 0; animation: zv-dots-in 600ms ease 1550ms forwards; }
.zv-dots > span {
...
@media (prefers-reduced-motion: reduce) {
  ...
  .zv-rule    { width: 8rem; }
  .zv-logo    { filter: drop-shadow(0 0 22px rgba(232,214,147,0.38)) drop-shadow(0 0 60px rgba(232,214,147,0.22)) !important; }
